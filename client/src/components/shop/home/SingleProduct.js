            ...
            <Fragment key={index}>
              <div className="product-card">
               ...
               Hayroo
				...
				<div style={{display: "flex"}} class="product-price">
        <TbCurrencyTaka size={30}/> {item.pPrice}.00 

          </div>
          <div style={{display: "flex"}} class="product-links">
            <AiOutlineShoppingCart onClick={(e) => history.push(`/products/${item._id}`)}
                  className="w-full heart object-cover object-center cursor-pointer"
                  src={`${apiURL}/uploads/products/${item.pImages[0]}`}
                  alt=""  size={40}/>
          </div>
          </div>
